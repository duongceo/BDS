...
namespace HappyRE.FileServiceProxy
{
    ...
    public class FileService : IFileService
    {
        ...
        public Response<FileResponseModel> UploadFile(Stream fileStream, string fileName, string code)
        {
            try
            {
                ...
                string resultPath = UploadFileEx(fileStream, path);
        ...
        private static string UploadFileEx(Stream file, string path)
        {
            var fileFormName = "file";
            var contenttype = "application/octet-stream";

            Uri uri = new Uri(Configs.TransferService_File);

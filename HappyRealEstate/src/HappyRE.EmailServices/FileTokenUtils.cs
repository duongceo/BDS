...
namespace HappyRE.EmailServices
{
    public class FileTokenUtils
    {
        ...
        public bool HasRefreshToken(string json) 
        {
            var token = Newtonsoft.Json.JsonConvert.DeserializeObject<GoogleApiToken>(json);
            if (string.IsNullOrEmpty(token.refresh_token) == false && token.refresh_token.Length > 10) return true;
            return false;
        }
    }
}

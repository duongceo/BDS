            ...
            if (Request.Files.Count > 0)
            {
                try
                {
                    ...
                    if (mimeType == ".png" || mimeType == ".jpg" || mimeType == ".bmp" || mimeType == ".jpeg")
                    {
                        ...
                        {
                            var newImg = ImageHelper.Resize(image, 2560, 1600, true);
                            newImg.Save(path);

                            //ImageFormat fm = mimeType == ".png" ? ImageFormat.Png : mimeType == ".bmp" ? ImageFormat.Bmp : ImageFormat.Jpeg;
                            //using (var stream = newImg.ToStream(fm))
                            //{
                            //    new HappyRE.Utils.GoogleStorageUtil().UploadToGooleCloud(path, mimeType, stream);
                            //}
                        }
                        else
                        {
                            file.SaveAs(path);
                            //new HappyRE.Utils.GoogleStorageUtil().UploadToGooleCloud(path, mimeType, file.InputStream);
                        }
                    }
                    else
                        file.SaveAs(path);

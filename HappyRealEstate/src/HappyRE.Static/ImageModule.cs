...
public class ImageModule : IHttpModule
{
    ...
    public void OnBeginRequest(object o, EventArgs args)
    {
        ...
        if ((ext.EndsWith(".jpg") || ext.EndsWith(".png") || ext.EndsWith(".bmp")) && app.Context.Request.Path.StartsWith("/img/s"))
        {
            ...
            //     }
            // }
            string pt = (app.Context.Request.Path.IndexOf("/thumb/")>0 || app.Context.Request.Path.IndexOf("/product/") > 0 )? @"/img/s(?<imageWidth>\d+)x(?<imageHeight>\d+)/(?<userid>\w+)/(?<folder>\w+)/img_(?<filename>\w+).(?<ext>\w..+)$": @"/img/s(?<imageWidth>\d+)x(?<imageHeight>\d+)/(?<userid>\w+)/img_(?<filename>\w+).(?<ext>\w..+)$";
            Regex pattern = new Regex(pt);
            Image newImg;
            try
            {
